...
FWHM = 5;
% ---------- Define RS scan  -----------------------
scan = 'MGT_run-02';   % get from json !!!
% ---------- Define output folder  -----------------------
out  = 'MGT2'; clean = 'AROMA';
        ...
        if strfind(Di(i).name,'sub')
            ...
            if strfind(Dii(1).name,'ses')
                ...
                if exist(fullfile(target, [out '_fsl'], Di(i).name, [clean '_func_data.nii.gz']),'file')
                    xsubs{cnt} = Di(i).name;
                    TMPdir{cnt} = fullfile(TMP, [out '_' clean], Di(i).name);
                    sub_raw{cnt} = fullfile(target, [out '_fsl'], Di(i).name, [clean '_func_data.nii.gz']);
                    targetdir{cnt} = fullfile(target, [out '_' clean], Di(i).name);
                    cat_dir{cnt} = fullfile(CAT, Di(i).name);
                    if exist(fullfile(RAWdirs, Di(i).name, 'fmap'),'dir')
                        FMdir_raw{cnt} = fullfile(RAWdirs, Di(i).name, 'fmap');
...
%         if exist(fullfile(TMPdir{sub}, ['Counfounds_' xsubs{sub} '.mat']),'file') && ...
%            ~exist(fullfile(targetdir{sub}, ['Counfounds_' xsubs{sub} '.mat']),'file')
                i = i + 1;
                idx(i) = sub;
        end
...
%         ' "RSsub_FM_AROMA(''' sub_raw{sub} ''' ,' '''' FMdir_raw{sub} ''' ,' '''' ...
...
%     system([call_matlab ' "RSsub_FM_AROMA(''' sub_raw{sub} ''' ,' '''' FMdir_raw{sub} ''' ,' '''' ...
%         xsubs{sub} ''' ,' '''' cat_dir{sub} ''' ,' '''' TMPdir{sub} ''',' '''' ...
%         FMdir{sub} ''',' '''' targetdir{sub} ''', ' '' num2str(TR) ...
%         '  ,' '' num2str(FWHM) '  ,' '' num2str(force) ' )" ' ],'-echo');
% end
...
parfor sub = 1:numel(xsubs)
    try
        RSsub_FM_AROMA(sub_raw{sub}, FMdir_raw{sub}, xsubs{sub}, cat_dir{sub}, ...
