...
datapath = fullfile(workdir, 'SingleSubjectAnalysis', MODELL_1st);  % Root-directory
    ...
    for scan = 1:numel(betas)
        ...
        if betas(scan)<10
            matlabbatch{1}.spm.stats.factorial_design.des.fblock.fsuball.fsubject(sub).scans{scan,1} = ...
                [datapath subjects{sub} '/con_000' int2str(betas(scan)) '.nii'];
        else
            matlabbatch{1}.spm.stats.factorial_design.des.fblock.fsuball.fsubject(sub).scans{scan,1} = ...
                [datapath subjects{sub} '/con_00' int2str(betas(scan)) '.nii'];
        end
    end
    ...
    for scan = 1:numel(betas)
        ...
        if betas(scan)<10
            matlabbatch{1}.spm.stats.factorial_design.des.fblock.fsuball.fsubject(sub2).scans{scan,1} = ...
                [datapath subjects2{sub} '/con_000' int2str(betas(scan)) '.nii'];
        else
            matlabbatch{1}.spm.stats.factorial_design.des.fblock.fsuball.fsubject(sub2).scans{scan,1} = ...
                [datapath subjects2{sub} '/con_00' int2str(betas(scan)) '.nii'];
        end
    end
