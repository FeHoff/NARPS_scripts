...
if force
    ...
    for sub = 1:numel(xsubs)
        if exist(sub_raw{sub},'file') && exist(cat_dir{sub},'dir') && ...
           ~exist(fullfile(targetdir{sub}, ['s5w' xsubs{sub} '.nii']),'file')
...
if numel(idx) == 0
    ...
    fprintf('Found %i unprocessed of %i subjects in %s ..\n', numel(idx), numel(xsubs), out);
end
...
%             TMPdir{sub}, FMdir{sub}, targetdir{sub}, TR, FWHM, force);
        RS_resmooth(sub_raw{sub}, FMdir_raw{sub}, xsubs{sub}, cat_dir{sub}, ...
            TMPdir{sub}, FMdir{sub}, targetdir{sub}, TR, FWHM, force);
%     end
end
