...
if force
    ...
    for sub = 1:numel(xsubs)
        if exist(sub_raw{sub},'file') && exist(cat_dir{sub},'dir') && ...
                ~exist(fullfile(targetdir{sub}, 'FIX_func_data.nii.gz'),'file')
...
%     system([call_slurm ' --error=slurm_log/RSfix_' xsubs{sub} ' local_plineEPI_FIX.sh ' ...
%         sub_raw{sub} ' ' xsubs{sub} ' ' cat_dir{sub} ' ' targetdir{sub} ' ' num2str(TR) ...
%         ' &' ],'-echo');
% end

for sub = 1:numel(xsubs)
  fprintf('Submit CAT12 job for subject %s \n', xsubs{sub});
    system([' local_plineEPI_FIX.sh ' sub_raw{sub} ' ' xsubs{sub} ' ' cat_dir{sub} ' ' ...
        targetdir{sub} ' ' num2str(TR) ' &' ],'-echo');
end