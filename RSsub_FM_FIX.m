...
if ~exist(fullfile(RSdir, ['w' xsub '_meanEPI.nii']), 'file') || force == 1
  ...
  % ---------- Affine registration into MNI space --------------------
    ...
    matlabbatch{1}.spm.spatial.coreg.estimate.ref{1}    = fullfile(strrep(targetdir,'_FIX',''), ['u' xsub '_sbref.nii']);
    % fullfile(strrep(strrep(RSdir,'RS_FIX','RS'),'_TEMP','2'),[xsub '_meanEPI.nii']);
    fils = dir(fullfile(origdir,'*.nii')); fils = fils(~[fils.isdir]);
    for file = 1:size(fils,1)
    ...
    mkdir(normdir);
    matlabbatch = {};
    matlabbatch{1}.spm.util.defs.comp{1}.sn2def.matname{1} = fullfile(strrep(targetdir, '_FIX', ''), ['u' xsub '_sbref_seg_sn.mat']);
    % fullfile(strrep(strrep(RSdir,'RS_FIX','RS'),'_TEMP','2'),[xsub '_meanEPI_seg_sn.mat']);
