...
datafolder = {'' '' '' '' '' ''};
datatype = {'CAT/12.5' 'CAT/12.5' 'MGT1' 'MGT2' 'MGT3' 'MGT4'};
cnt = 0; Pcnt = 0;
...
for sub = 1:numel(xsub)
    ...
    datafile = {['mri/m0wp1' xsub{sub} '.nii'] ['surf/mesh.thickness.resampled_32k.' xsub{sub} '.gii'] ...
        ['s8w' xsub{sub} '.nii'] ['s8w' xsub{sub} '.nii',1] ['s8w' xsub{sub} '.nii',1] ['Smooth_8mm/sw' xsub{sub} '_00001.nii']};
    ...
    if inpt == 1
        if exist(fullfile(subjects{sub},datafile{data}),'file')
            ...
            if data == 2
                ...
                img1 = spm_select('expand',fullfile(subjects{sub},datafile{data}));
