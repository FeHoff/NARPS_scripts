function RSsub_FM_AROMA(sub_raw, FMdir_raw, xsub, cat_dir, RSdir, FMdir, targetdir, TR, FWHM, force)
cwd = pwd;
...
if ~exist(fullfile(RSdir, ['w' xsub '_meanEPI.nii']), 'file') || force == 1
  % ---------- Normalise EPIs using transformation from RS folder -----
  % ---------- Affine registration into MNI space --------------------
    ...
    matlabbatch{1}.spm.spatial.coreg.estimate.source{1} = fullfile(RSdir,[xsub '_meanEPI.nii']);
    matlabbatch{1}.spm.spatial.coreg.estimate.ref{1}    = fullfile(strrep(targetdir,'_AROMA',''), ['u' xsub '_sbref.nii']);
    % fullfile(strrep(strrep(RSdir,'RS_AROMA','RS'),'_TEMP','2'),[xsub '_meanEPI.nii']);
    fils = dir(fullfile(origdir,'*.nii')); fils = fils(~[fils.isdir]);
    for file = 1:size(fils,1)
    ...
    mkdir(normdir);
    matlabbatch = {};
    matlabbatch{1}.spm.util.defs.comp{1}.sn2def.matname{1} = fullfile(strrep(targetdir, '_AROMA', ''), ['u' xsub '_sbref_seg_sn.mat']);
    % fullfile(strrep(strrep(RSdir,'RS_AROMA','RS'),'_TEMP','2'),[xsub '_meanEPI_seg_sn.mat']);
    ...
    matlabbatch{1}.spm.util.defs.out{1}.pull.fnames{1,1} = fullfile(RSdir,[xsub '_meanEPI.nii']);
...
% end
% 
% %%% redo smoothing with 8mm -- IMPLEMENT A LOOP with previous !!!
