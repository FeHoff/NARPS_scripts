...
datadir = fullfile('/data/BnB_USER/Langner/NARPS/analysis/SPM/SingleSubjectAnalysis/NARPS_gain1st');
...
data = spm_input('Choose data to QC ->','!+1','b','mri|surf|NARPS_gain1st|MGT2|MGT3|MGT4',[1 2 3 4 5 6],1,'Select mode');
datafolder = {'' '' '' '' '' ''};
datatype = {'CAT/12.5' 'CAT/12.5' '' 'MGT2' 'MGT3' 'MGT4'};
cnt = 0; Pcnt = 0;
...
for sub = 1:numel(xsub)
    ...
    datafile = {['mri/m0wp1' xsub{sub} '.nii'] ['surf/mesh.thickness.resampled_32k.' xsub{sub} '.gii'] ...
        ['mask.nii'] ['w' xsub{sub} '_meanEPI.nii'] ['w' xsub{sub} '_meanEPI.nii'] ['w' xsub{sub} '_meanEPI.nii']};
