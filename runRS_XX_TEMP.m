...
TMP    = fullfile('/data/BnB_TEMP/Data_NARPS/Derivatives');
                    ...
                    if exist(fullfile(subdir, Dii(ii).name, 'func'),'dir')
                        ...
                        if numel(fil) > 0
                            ...
                            if exist(fullfile(subdir, Dii(ii).name, 'fmap'),'dir')
                                ...
                                FMdir{cnt} = fullfile(TMP, 'FM', Di(i).name, Dii(ii).name);
                            else
                                FMdir_raw{cnt} = '';
                                FMdir{cnt} = '';                   
                            end
                ...
                if exist(fullfile(subdir, 'func'),'dir')
                    ...
                    if numel(fil) > 0
                        ...
                        if exist(fullfile(subdir, 'fmap'),'dir')
                            ...
                            FMdir{cnt} = fullfile(TMP, 'FM', Di(i).name);
                        else
                            FMdir_raw{cnt} = '';
                            FMdir{cnt} = '';                   
                        end
                ...
                if strfind(Dii(1).name,'ses')
                    for ii=1:size(Dii,1)
                        if exist(fullfile(subdir, Dii(ii).name, 'func'),'dir')
                            ...
                            if numel(fil) > 0
                                ...
                                if exist(fullfile(subdir, Dii(ii).name, 'fmap'),'dir')
                                    FMdir_raw{cnt} = fullfile(subdir, Dii(ii).name, 'fmap');
                                    FMdir{cnt} = fullfile(TMP, site, 'FM', Di(i).name, Dii(ii).name);
                                else
                                    FMdir_raw{cnt} = '';
                                    FMdir{cnt} = '';                   
                                end
                    ...
                    if exist(fullfile(subdir, 'func'),'dir')
                        ...
                        if numel(fil) > 0
                            ...
                            if exist(fullfile(subdir, 'fmap'),'dir')
                                FMdir_raw{cnt} = fullfile(subdir, 'fmap');
                                FMdir{cnt} = fullfile(TMP, site, 'FM', Di(i).name);
                            else
                                FMdir_raw{cnt} = '';
                                FMdir{cnt} = '';                   
                            end
