...
# Standard.RData - for use on more "standard" FMRI datasets / analyses; e.g., TR=3s,
# Resolution=3.5x3.5x3.5mm, Session=6mins, default FEAT preprocessing (including
# default spatial smoothing).

# HCP_hp2000.RData for use on "minimally-preprocessed" 3T HCP-like datasets, e.g.,
# TR=0.7s, Resolution=2x2x2mm, Session=15mins, no spatial smoothing, minimal
# (2000s FWHM) highpass temporal filtering.

# HCP7T_hp2000.RData for use on "minimally-preprocessed" 7T HCP-like datasets, e.g.,
# TR=1.0s, Resolution=1.6x1.6x1.6mm, Session=15mins, no spatial smoothing, minimal
# (2000s FWHM) highpass temporal filtering.

# WhII_MB6.RData derived from the Whitehall imaging study, using multiband x6 EPI
# acceleration: TR=1.3s, Resolution=2x2x2mm, Session=10mins, no spatial smoothing,
# 100s FWHM highpass temporal filtering.
...
# UKBiobank.RData derived from fairly HCP-like scanning in the UK Biobank imaging
# study: 40 subjects, TR=0.735s, Resolution=2.4x2.4x2.4mm, Session=6mins, no
# spatial smoothing, 100s FWHM highpass temporal filtering.
...
function prepro	{
	...
	if [ -f $EPIRAW ] && [ -f $CATDIR/mri/rmi${subject}_affine.nii ] && [ ! -f $FUNCDIR/FIX_func_data.nii.gz ]; then
			date >> $subLog
    ...
    if [ -f $EPIRAW ]; then
		    ...
		    if [ ! -f $FUNCDIR/filtered_func_data.nii.gz ] ; then
			    ...
			    run fslmaths $FUNCDIR/mc/prefiltered_func_data_mcf -mul $INTTHRESH $FUNCDIR/prefiltered_func_data_intnorm
					imcp $FUNCDIR/prefiltered_func_data_intnorm $FUNCDIR/filtered_func_data
	...
	if [ -f $FUNCDIR/filtered_func_data_clean.nii.gz ] && [ -f $FUNCDIR/conf/FIX_meanWM.txt ]; then
		...
		if [ -f $FUNCDIR/filtered_func_data.nii.gz ] && [ ! -f $FUNCDIR/filtered_func_data_clean.nii.gz ]; then
		  ...
		  #                                      default temporal filtering of motion parameters
			...
			if [ ! -f $FUNCDIR/filtered_func_data_clean.nii.gz ] && [ -d $FUNCDIR/filtered_func_data.ica ]; then
				echo "FIX-ing EPIs of $subject"	>> $subLog
				#run $FIXDIR/fix $LOCDIR $trainData 20 -m
				if [ ! -f $LOCDIR/fix/features.csv ]; then
					# extract features
					run $FIXDIR/fix -f $LOCDIR
				fi
				if [ ! -f $LOCDIR/fix4melview_WhII_Standard_thr20.txt ]; then
					# classify components
					run $FIXDIR/fix -c $LOCDIR $trainData 20
				fi
				# apply cleanup 'fix4melview_WhII_Standard_thr20.txt'
				run $FIXDIR/fix -a $LOCDIR/fix4melview_*.txt
				# run $FIXDIR/fix -a $LOCDIR/fix4melview_*.txt -m
			fi

			if [ -f $FUNCDIR/filtered_func_data_clean.nii.gz ] && [ ! -f $FUNCDIR/FIX_func_data_MNI152.nii.gz ]; then
				...
				run immv $LOCDIR/filtered_func_data_clean $FUNCDIR/FIX_func_data
				### careful NOT to remove FUNCDIR !!!
				# run rm -R $LOCDIR
			fi

			if [ -f $FUNCDIR/FIX_func_data_MNI152.nii.gz ] && [ ! -f $FUNCDIR/conf/FIX_meanWM.txt ]; then
				# calculates mean GS, GM, WM & CSF (for noise regression)
