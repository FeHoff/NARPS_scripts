...
for sub= 1:numel(subjects)
    if ~exist(fullfile(results_folder,subjects{sub},'con_0008.nii'),'file') && ...
      exist(fullfile(results_folder,subjects{sub},'con_0004.nii'),'file')
        % try
            addCon(fullfile(results_folder, subjects{sub}));
        % end 
        cd(cwd)
    end
end
